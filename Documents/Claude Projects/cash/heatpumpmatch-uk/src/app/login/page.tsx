...
export default function LoginPage() {
  ...
  function handleSubmit(e: FormEvent<HTMLFormElement>) {
    ...
    // Simulate async sign-in (no actual auth)
    setTimeout(() => {
        ...
        <div className="w-full max-w-md">
          ...
          <div className="bg-white rounded-2xl border border-hpm-border shadow-sm p-8">
            <div className="text-center mb-7">
              <h2 className="font-heading text-2xl font-bold text-hpm-text">Welcome back</h2>
              <p className="mt-2 text-sm text-hpm-text-secondary">
                Sign in to manage your installer listing
              </p>
            </div>
            ...
            {error && (
              <div
                role="alert"
                className="mb-5 rounded-lg bg-red-50 border border-red-200 px-4 py-3 text-sm text-hpm-error"
              >
                {error}
              </div>
            )}
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-5" noValidate>
              ...
              <div>
                <div className="flex items-center justify-between mb-1.5">
                  <label
                    htmlFor="password"
                    className="block text-xs font-semibold text-hpm-text-secondary uppercase tracking-wide"
                  >
                    Password
                  </label>
                  <Link
                    href="/forgot-password"
                    className="text-xs text-hpm-primary hover:underline hover:text-hpm-primary-dark transition-colors"
                  >
                    Forgot password?
                  </Link>
                </div>
              ...
              <button
                type="submit"
                disabled={loading}
                className="w-full inline-flex items-center justify-center rounded-lg bg-hpm-primary hover:bg-hpm-primary-dark text-white font-semibold text-sm px-4 py-2.5 transition-colors disabled:opacity-60 disabled:pointer-events-none mt-1"
              >
                {loading ? (
                  <>
                    <svg
            ...
            <p className="mt-6 text-center text-sm text-hpm-text-secondary">
              New here?{" "}
              <Link href="/signup" className="text-hpm-primary hover:underline font-medium">
                Create an account
              </Link>
            </p>
          </div>
